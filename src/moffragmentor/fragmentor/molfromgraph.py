            ...
            for neighbor_index in neighbor_indices:
                if (neighbor_index not in new_positions_cart) & (neighbor_index in mol_idxs):
                    ...
                    if neighbor_index in new_positions_cart: 
                        ...
                        if np.linalg.norm(new_positions_cart[neighbor_index] - new_positions_cart[current_index]) > VestaCutoffDictNN._lookup_dict[species_a][species_b]: 
                            logger.warning("Warning: neighbor_index {} is already in new_positions_cart, but the distance is too large. "
                            "Will add an additional site. This is unusual and not well tested".format(neighbor_index))
